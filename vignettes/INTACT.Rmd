...
association studies and colocalization analysis prioritizes molecular pathways 
of complex traits." bioRxiv (2022).
...
rst <- INTACT::intact(GLCP_vec=INTACT::simdat$GLCP, 
                      z_vec=INTACT::simdat$TWAS_z)
```

The ```intact``` function takes a vector of gene-level colocalization 
probabilites ```GLCP_vec``` and TWAS scan z-scores ```z_vec```. It outputs 
...
```{r}
rst1 <- INTACT::intact(GLCP_vec=INTACT::simdat$GLCP, 
                       prior_fun=INTACT::expit, 
                       z_vec = INTACT::simdat$TWAS_z,
                       t = 0.02,D = 0.09)
rst2 <- INTACT::intact(GLCP_vec=INTACT::simdat$GLCP, 
                       prior_fun=INTACT::step, 
                       z_vec = INTACT::simdat$TWAS_z,
                       t = 0.49)
rst3 <- INTACT::intact(GLCP_vec=INTACT::simdat$GLCP, 
